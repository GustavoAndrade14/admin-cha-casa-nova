...
export function useProdutos() {
    ...
    const fetchProdutos = useCallback(async () => {
        ...
        try {
            ...
            const response = await axios.get('http://localhost:4000/api/produtos', {
    ...
    const toggleStatus = useCallback(async (id: number, currentStatus: boolean) => {
        ...
        try {
            const response = await axios.patch(`http://localhost:4000/api/produtos/${id}/toggle`);
    ...
    const updateMultipleStatus = useCallback(async (updates: { id: number; unavailable: boolean }[]) => {
        ...
        try {
            const response = await axios.patch('http://localhost:4000/api/produtos/batch/atualizar', {
                updates
            });
